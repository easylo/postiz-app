// Copy of v1.0.5, differing only in how a successful post is recorded: it calls
// updatePostWithNote so a provider can say it published something other than
// what was asked — TikTok downgrading a public post to private, or dropping the
// media in an inbox instead of publishing it.
//
// A new version rather than an edit: v1.0.5 is already running, and both the
// workflow code and an activity's parameters are frozen once executions exist.
  ...
  for (let i = 0; i < postsList.length; i++) {
    ...
    for (const _ of iterate) {
      try {
        ...
        // mark post as successful — with the provider's note, when it published
        // something other than what was asked
        await updatePostWithNote(
          postsList[i].id,
